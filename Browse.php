...
if(isset($searchbar)){
    ...
    $stmt->bindValue(3, $resultsperpage, PDO::PARAM_INT);
                        ...
                        $q = $db->prepare("SELECT * FROM users WHERE discord_verified = 1 AND discord_id = :id");
                        $q->bindParam(':id', $searchbar, PDO::PARAM_STR);
                        $q->execute();
                        $discordusr = $q->fetch();
                        if($discordusr)
                            array_push($users, $discordusr);
                        ?>
<div id="Body">
...
if(!$banned) {
    ...
    <td>
          <img src='/img/user/".$row['id'].".png?rand=".random_int(1,999999999999999999)." title='".addslashes(htmlspecialchars($row['username']))."' width='60' height='67'>
    </td>
